...
static void process_msg(void) {
        ...
        case EC_HELLO_SERVER:
	        metrics.num_cpus = data[1];
	        metrics.cpu = (struct cpu_metrics*) kmalloc(
			        sizeof(struct cpu_metrics) * metrics.num_cpus, GFP_KERNEL);
        ...
        case EC_CPU_ERROR:
	        for (i = 0; i < metrics.num_cpus; i++) {
		        metrics.cpu[i].sys = (int) data[1 + i * 2];
		        metrics.cpu[i].usr = (int) data[2 + i * 2];
	        }
	        break;
        case EC_MEM_ERROR: {
	        for (i = 0; i < metrics.num_cpus; i++) {
		        metrics.cpu[i].iowait = (int) data[1 + i];
	        }
	        metrics.blk = (int) data[1 + metrics.num_cpus];
	        metrics.pswpout = ((int) data[2 + metrics.num_cpus] << 8);
	        metrics.pswpout |= ((int) data[3 + metrics.num_cpus]);
	        metrics.memfree = (int) (data[4 + metrics.num_cpus] << 16);
	        metrics.memfree |= ((int) data[5 + metrics.num_cpus] << 8);
	        metrics.memfree |= (int) data[6 + metrics.num_cpus];
        }
	        break;
        case EC_PROC_ERROR: {
	        metrics.run = (int) data[1];
	        metrics.blk = (int) data[2];
        }
        }
...
static int process_data() {
        ...
        case EC_MEM_ERROR: {
	        ...
	        if (metrics.blk >= THRESHOLD_BLK && check_abnormal_memory_consumption()) { 
		        return 1;
	        }
        }
	        break;
        case EC_PROC_ERROR: {
	        if (metrics.run >= THRESHOLD_RUN * metrics.num_cpus && metrics.blk >= THRESHOLD_BLK)
		        return 1;
                else return check_abnormal_runtime();
        }
        }
        return 0;
}
